...
def update_balls(dt):
    """Move all active balls and check collisions with enemies, mini bosses & boss."""
    import mini_boss
    ...
    for ball_obj in thrown_balls:
        ...
        # --- Collision with mini bosses ---
        current_floor = floors.get_current_floor()
        if current_floor in mini_boss.mini_bosses:
            mb = mini_boss.mini_bosses[current_floor]
            if mb["active"] and not mb["defeated"]:
                dx = ball_obj.pos[0] - mb["pos"][0]
                dy = ball_obj.pos[1] - mb["pos"][1]
                dz = ball_obj.pos[2] - mb["pos"][2]
                dist = math.sqrt(dx*dx + dy*dy + dz*dz)
                
                if dist < BALL_RADIUS + mini_boss.MINI_BOSS_RADIUS:
                    mini_boss.hit_mini_boss(current_floor)
                    ball_obj.active = False
